...
public class BSTree {
	...
	public void children(int key) {
		Node r = search(root,key);
		if(r.left!=null)
			System.out.println("left node : "+ r.left.data);
		if(r.right!=null)
			System.out.println("right node : "+ r.right.data);	
		
	}
	
	public void siblingsPair(Node trav) {
		if(trav==null)
			return;
		if(trav.left!=null)
			siblingsPair(trav.left);
		if(trav.right!=null)
			siblingsPair(trav.right);
		if(trav.left!=null && trav.right!=null) {
			System.out.print("("+trav.left.data+","+trav.right.data+") ");
		}
			
	}
	public void siblingsPair() {
		siblingsPair(root);
	}
}
